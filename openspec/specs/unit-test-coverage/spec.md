...
## Purpose
Provide unit-test coverage that runs without a live SMB server. This capability defines serialization tests for `SMB2Manager` (NSCoding, Codable, NSCopying, and invalid-input handling), public-interface tests for `SMB2Client`, unit tests for the file-monitoring public types, and documentation of the build prerequisites required before the suite can compile.
