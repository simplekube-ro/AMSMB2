# disconnect-timeout-tests Specification

## Purpose
Integration tests covering disconnect behavior (graceful/non-graceful) and operation timeout handling against a live SMB server. These tests exercise the graceful disconnect drain mechanism, non-graceful teardown of in-flight operations, post-disconnect error reporting, reconnect round-trip data integrity, and the per-operation timeout path — critical resilience paths that affect production reliability.
