# request-pipelining Specification

## Purpose
Keep multiple SMB2 requests in-flight over a single TCP connection, bounded by server-granted credits, to hide round-trip latency. Pipelined reads reassemble in offset order, pipelined writes track acknowledgments and cancel on failure, and independent operations on different handles do not block one another.
