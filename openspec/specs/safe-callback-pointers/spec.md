# safe-callback-pointers Specification

## Purpose
Replace undefined-behavior pointer passing with `Unmanaged`-based safe pointer management for all async SMB2 operations. Passing `CBData` references to libsmb2's C callback APIs via `withUnsafeMutablePointer(to:_:)` yields a pointer that is only valid within the closure, yet libsmb2 stores it and invokes the callback later during `wait_for_reply`'s poll loop — leaving the pointer dangling. This caused `monitorItem` (SMB2 Change Notify) to crash because its callback fires after an extended wait. Using `Unmanaged<CBData>.passUnretained(_:).toOpaque()` provides a stable pointer that remains valid for the full operation.
