...
## Purpose
Define when and how the SMB-over-QUIC transport is selected and validated: QUIC is explicit opt-in only (`SMBTransportKind.quic`), targets non-numeric hostnames on UDP 443 by default, arms a dedicated finite connect timeout from `SMBQUICConfiguration.connectTimeout`, never silently falls back to another transport, and is configured through `SMB2Manager` properties whose values are snapshotted immutably at connect and preserved by copying/coding per design D6.
