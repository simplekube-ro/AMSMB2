# validated-client-accessor Specification

## Purpose
Provide an `smbClient` getter that validates both client existence and live connection state under `connectLock`, preventing a TOCTOU race with `connectShare`/`disconnectShare` and avoiding return of a stale or destroyed client.
