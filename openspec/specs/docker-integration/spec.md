...
## Purpose
Provide a reproducible, containerized SMB test server so integration tests can run against a live SMB share. This capability defines the Docker Compose service that runs Samba, the Samba configuration for the writable test share, an orchestration script that manages the full Docker lifecycle (startup, health check, test execution, teardown), and a `Makefile` target that invokes it.
