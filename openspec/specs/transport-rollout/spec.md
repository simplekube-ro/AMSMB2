# transport-rollout Specification

## Purpose
Govern the staged rollout of the NIO TCP transport: validate it through the Docker-based Samba integration suite across the full acceptance matrix (connect, NTLM auth, directory listing, large read, large write, cancel/timeout), then flip the Apple default to `TCPTransportApple` (the seam) once acceptance passes, and compile out the now-dead Apple legacy `DispatchSource`/`SocketMonitor` path while retaining it on Linux (the sole remaining consumer). CI SHALL exercise both the seam leg (macOS) and the legacy leg (Linux).
