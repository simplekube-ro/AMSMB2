...
## Purpose
Expand integration-test coverage of the `SMB2Manager` public API against a live SMB server. This capability defines `SMB2IntegrationTests` coverage for append, generic `removeItem`, server-side `copyContentsOfItem`, `echo` liveness, progress-handler cancellation, common error paths, and the public `smbClient` accessor.
