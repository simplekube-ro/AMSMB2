# thread-safe-context-access Specification

## Purpose
Enforce that all `smb2_context` access goes through `withThreadSafeContext`, making the lock discipline compiler-verified via `private` access control, and keep construction and raw-handle typealiases internal so they cannot leak into public API.
