# stream-eof-semantics Specification

## Purpose
Define the testable contract that `AsyncInputStream` reports end-of-stream only after its underlying `AsyncSequence` producer is exhausted — never when the consumer transiently out-paces the prefetch producer. A transient buffer drain reports would-block (`-1`) so the consumer retries, a producer error surfaces as a thrown error, and a streamed upload of `N` bytes (including `N > 5 MiB`) transfers all `N` bytes and round-trips byte-equal, eliminating the silent ~5 MiB truncation in `write(stream:toPath:)`.
