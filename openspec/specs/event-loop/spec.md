...
## Purpose
Confine all `smb2_context` access to a single dedicated event loop thread, monitor the socket with `DispatchSource` instead of a polling loop, accept work from any thread via a thread-safe MPSC submission queue, bridge libsmb2's callback model to Swift async/await with `CheckedContinuation`, and shut down gracefully without leaking suspended continuations or the `smb2_context`.
