...
## Purpose
Reduce per-read allocation overhead by reusing pre-allocated `Data` buffers instead of allocating a new zeroed `Data` on every read. `SMB2Client` maintains a bounded pool of reusable buffers, read operations check buffers in and out of the pool, and the result is delivered without a second copy.
