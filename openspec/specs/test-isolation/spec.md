...
## Purpose
Ensure the test suite runs cleanly regardless of environment. Tests that require a live SMB server SHALL skip gracefully when no server is configured, pure unit tests SHALL always execute, and lazy environment-derived vars SHALL never force-unwrap in a way that crashes the test process.
