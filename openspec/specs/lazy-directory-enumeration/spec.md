# lazy-directory-enumeration Specification

## Purpose
Enumerate directories on demand instead of materializing every entry up front. A new `AsyncThrowingStream`-based listing API yields entries lazily, the existing eager `contentsOfDirectory(atPath:)` is reimplemented on top of it for backward compatibility, recursive listing descends lazily, and the full-scan `count` is eliminated.
