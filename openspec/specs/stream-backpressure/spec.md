...
## Purpose
Bound the peak memory of `AsyncInputStream` by applying backpressure to its prefetch task. A high-water mark suspends prefetching until the consumer drains the buffer below a low-water mark, keeping peak memory near the high-water mark plus one chunk regardless of total stream size, with both marks configurable.
