...
## Purpose
Define `QUICTransportApple`, the public `SMBTransport` conformer that carries SMB2 bytes over a single bidirectional Network.framework QUIC stream on Apple platforms (iOS 15 / macOS 12 / macCatalyst 15 / tvOS 15 / watchOS 8 / visionOS 1+): the ALPN "smb"/TLS 1.3/SNI handshake, the strictly one-shot connect lifecycle with atomic outcome claim and duty assignment, the seam's receive chunk/EOF conventions, the first-caller-owned close lifecycle, and the mutually exclusive, secure-by-default TLS trust policy (design D5/D7/D8).
