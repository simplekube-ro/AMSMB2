...
- [ ] 1.1 Table-driven tests for numeric-host classification (`isNumericHost(_:)`): rejected —
...
- [ ] 1.3 Tests for kind dispatch and selector exactness: `.automatic` never yields QUIC;
...
- [ ] 1.4 Boundary tests then implementation for the QUIC connect-timeout contract (design
      ...
      unchanged)
- [ ] 1.5 Tests then implementation for the D12 bridge-ownership handoff in
...
- [ ] 2.1 Tests for `SMBQUICConfiguration`: `trustPolicy` defaults to `.system`,
...
- [ ] 2.2 Implement `QUICTransportApple` skeleton: availability-gated final class
...
- [ ] 2.3 Implement the D7 connect state machine with the **atomic outcome claim**: a
      ...
      only
- [ ] 2.4 Implement handshake parameter wiring: `NWParameters(quic:)` with ALPN `"smb"`,
...
- [ ] 2.5 Implement `send(_:)`/`receive()` over the single bidirectional stream with re-arming
...
- [ ] 2.6 Wire `.quic` in the `Context.swift` kind dispatch to construct
...
- [ ] 3.2 Tests then implementation for serialization AND copying: `transportKind` round-trips
      ...
      of the original
- [ ] 3.3 Implement the D11 Swift-only Objective-C decision: verify no `@nonobjc` is needed
...
- [ ] 3.4 Linux routing tests (run under `make linuxtest`, per design D6): the manager takes
...
- [ ] 5.1 Update `docs/API.md` per the `api-reference` delta (new types incl.
...
- [ ] 5.3 Pre-archive sweep: dead-code check (every new symbol has a call site outside its
