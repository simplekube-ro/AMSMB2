...
- **One-shot connect ownership.** `connect(host:port:)` is strictly one-shot per instance: the first call atomically reserves the single attempt (before trust resolution or driver construction), and every other call fails promptly — `EALREADY` while in flight or after a failed attempt (retry is unsupported; build a fresh transport, as `SMB2Client` does), `EISCONN` after success, `ECONNABORTED` after `close()` — without constructing a driver, arming a timer, or touching the owning attempt's state.
...
- **Close lifecycle.** `close()` runs `open → closing → closed`: the first caller becomes the teardown owner, performs the resource teardown on a dedicated teardown queue (driver cancel exactly once, parked-receive resumption, connect-claim resolution), then waits for any in-flight connect work — a parked committed-start teardown, a still-running `start()` tail (including `.ready` winning mid-`start()`, where close cancels the ready driver immediately but still waits for the tail), or the deadline-arming tail — before publishing fully-closed and resuming every concurrent caller. Every `close()` caller observes completed teardown: a call arriving during teardown waits for it; only a call after full completion returns immediately. Once any `close()` has returned, no start, cancel, receive-arm, or armed timer remains outstanding.
