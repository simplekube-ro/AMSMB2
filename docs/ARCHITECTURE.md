...
sequenceDiagram
    ...
    opt Apple seam (transportKind != nil)
        Client->>Transport: connect(host:port:onReceive:) — eager TCP handshake
...
- **Readiness signalling** differs by platform but converges on `smb2_service()`:
  - **Apple (seam):** the transport pushes each received chunk into the bridge's store on its own network queue, which fires an inbound-ready signal; a (debounced) `eventLoopQueue.async` then calls `serviceContextForSeam()` → `smb2_service()`. There is no socket fd (`smb2_get_fd == -1`). See [Transport Layer](#transport-layer).
...
- **Apple:** the transport pushes received bytes into the `TransportBridge` store, firing an inbound-ready signal that dispatches `serviceContextForSeam()` → `smb2_service()` on the event loop queue (no socket fd). **Linux:** `DispatchSource` monitors socket readability/writability and `handleSocketEvent()` calls `smb2_service()`. Either way, `smb2_service()` invokes `generic_handler` for completed operations
...
flowchart TB
    ...
    subgraph bridge["TransportBridge (Apple)"]
        ...
        IN["Inbound FIFO + head cursor<br/>deliverInbound appends; cRecv drains synchronously"]
    end
    ...
    Proto -->|"onReceive (network queue)"| IN -->|"cRecv"| EXT
    Proto --> TCP
    Proto --> QUIC
    TCP <-->|"TCP/445"| Server
...
Both conformers honor the identical seam contract (pushed per-chunk delivery, empty-`Data` EOF, `POSIXError` mapping, silent local teardown), so `TransportBridge` and the servicing loop are unchanged between them — QUIC is a different wire under the same byte-pipe.

### Types

| Type | Visibility | Role |
|------|-----------|------|
| `SMBTransport` | `public protocol` | The seam. Async `connect(host:port:onReceive:)` / `send(_:)` / `close()` over `Data`. No SwiftNIO, no libsmb2 — unit-testable in isolation, shared by the TCP and QUIC conformers. Inbound is **push**: the receiver is a `connect` parameter, invoked per chunk on the transport's own serial queue; empty `Data` signals graceful EOF and a `POSIXError` signals abnormal loss (both terminal, and nothing is delivered once `close()` has begun). |
| `SMBTransportKind` | `public enum` | Selects a transport: `.tcp`, `.quic`, `.automatic` (never QUIC). |
| `TCPTransportApple` | `public final class` | Concrete `SMBTransport` over `NIOTransportServices`. Converts `Data` ↔ `ByteBuffer` internally; forwards each inbound chunk to the receiver as it arrives on the channel's event loop (`InboundForwardingHandler`, no transport-side buffer, zero-length reads skipped); maps NWError/ChannelError → `POSIXError`. One instance = one connection lifetime. |
...
| `SMBQUICCertificateProbe` | `public enum` (caseless) | `fetchServerCertificateChain(server:timeout:)` — one capture-only SMB-over-QUIC TLS handshake that returns the server's DER chain leaf first as `[Data]` without trusting the peer or creating an SMB session; a thin classifier over an unchanged `QUICTransportApple.connect` (Apple, availability-gated; Linux throws `ENOTSUP`). |
...
- **Eager connect ordering.** The bridge connects the transport (`TCPTransportApple.connect(host:port:onReceive:)`, which also installs the bridge as the connection's receiver) on the caller's task **before** libsmb2 begins NEGOTIATE. `ext_connect` fires NEGOTIATE synchronously on a `>= 0` return, so the channel must already be live or the first `send` would fail with `ENOTCONN`. A transport-connect failure surfaces here as a thrown `POSIXError` (`.ECONNREFUSED`/`.ETIMEDOUT`), with nothing installed and nothing leaked.
...
- **Inbound.** The bridge hands the transport its receiver (`deliverInbound`) when it connects it, so there is **no inbound task**: the transport invokes it on its own network queue for each chunk, for graceful EOF and for abnormal loss, and it appends to an inbound FIFO (a list of `Data` chunks with a head cursor — O(bytes copied), no tail shifting) with the bytes already copied at the C/NIO boundary. The C `recv` callback drains that FIFO synchronously: bytes available → copy up to `max_len`; empty but open → libsmb2 would-block signal; empty + EOF → `0`; errored → negative. Deliveries after the bridge has closed are ignored.
- **Inbound-ready signal.** Each delivery fires an inbound-ready handler — before the delivery returns, on the network queue — that (after a debounce) does `eventLoopQueue.async { serviceContextForSeam() }`, calling `smb2_service` with `revents` derived from `smb2_which_events`. This is the Apple analogue of a `DispatchSource` read event. Registering that handler also fires it once if the store is already non-empty, so a delivery landing between the eager connect and the registration is never a lost wakeup; the outbound pump is deliberately started *after* the registration so no NEGOTIATE can leave before it.
...
- **One-shot connect ownership.** `connect(host:port:onReceive:)` is strictly one-shot per instance: the first call atomically reserves the single attempt (before trust resolution or driver construction), and every other call fails promptly — `EALREADY` while in flight or after a failed attempt (retry is unsupported; build a fresh transport, as `SMB2Client` does), `EISCONN` after success, `ECONNABORTED` after `close()` — without constructing a driver, arming a timer, touching the owning attempt's state, or replacing its receiver.
- **Connect state machine.** Because the eager `bridge.connect` runs *before* libsmb2's cancellation/timeout machinery is installed, connect is self-contained. A lock-protected outcome claim (`connecting → ready | failed`) funnels every completion — `.ready`, `.failed`, task cancellation, `close()`, deadline expiry — through one critical section: exactly one path wins the claim and is *assigned* the cleanup duty, which it then performs outside the lock; paths that lose the claim do nothing (a losing task-cancellation never cancels the `NWConnection`). The one case where the winner does not execute its own duty is a loss landing in the commit-to-start window: it parks the outcome, and the path that called `driver.start()` cancels the deadline and the connection after `start()` returns, then resumes — so a connection is never cancelled before its start side effect has happened. The whole attempt after the continuation store (deadline arming, commit, `driver.start()`, handoff) runs on a dedicated serial queue (never a Swift cooperative-pool thread) and is tracked as in-flight work until it fully finishes; a loser that consumes the claim while the deadline is still arming is backstopped by a post-`schedule` re-check that cancels the late-armed timer, so no timer survives a terminal outcome or a completed close. `NWConnection` states are handled explicitly (`.setup`/`.preparing` progress; `.waiting` classified by error class at the connection-driver seam — a **transient** wait (no route, DNS, refused, any non-TLS condition) is non-terminal, its error recorded, while a **fatal** wait (a TLS handshake/trust rejection, which no path change can heal) claims the connect outcome as failure exactly as `.failed` does, without waiting for the deadline; `.ready` success; `.failed` mapped `POSIXError`; `.cancelled` terminal ack). A deterministic, always-armed deadline is sourced from the validated `SMBQUICConfiguration.connectTimeout` (independent of `SMB2Manager.timeout`). Error contract: task cancel → `CancellationError`; `close()` while connecting → `ECONNABORTED`; deadline → `ETIMEDOUT`; `.failed` and a fatal `.waiting` → mapped `POSIXError` (a TLS rejection is `EPROTO` carrying the Security `OSStatus` as an `NSOSStatusErrorDomain` `NSUnderlyingErrorKey`).
- **Close lifecycle.** `close()` runs `open → closing → closed`: the first caller becomes the teardown owner, performs the resource teardown on a dedicated teardown queue (driver cancel exactly once, inbound delivery terminated, connect-claim resolution), then waits for any in-flight connect work — a parked committed-start teardown, a still-running `start()` tail (including `.ready` winning mid-`start()`, where close cancels the ready driver immediately but still waits for the tail), or the deadline-arming tail — before publishing fully-closed and resuming every concurrent caller. Every `close()` caller observes completed teardown: a call arriving during teardown waits for it; only a call after full completion returns immediately. Once any `close()` has returned, no start, cancel, inbound delivery, or armed timer remains outstanding, and the receiver has been released.
- **Established-connection lifecycle (recorded causes).** After `.ready`, teardown is discriminated by cause: **local close** — `close()` records the local-close cause under the lock *before* `NWConnection.cancel()`, so the resulting `.cancelled` is a no-op ack and the receiver is not invoked at all (matching `TCPTransportApple.signalClosed()`); **peer graceful EOF** — one empty-`Data` delivery from a remote stream close; **abnormal loss** — an unsolicited post-ready `.failed`, or a `.cancelled` with no recorded local-close cause, delivers one mapped `POSIXError` failure. The first lock-protected transition out of `ready` wins; a recorded local-close result is never overwritten. A single lock-guarded terminal-once flag spans all four producers (peer EOF, receive-side failure, post-ready `.failed`, post-ready `.cancelled`) and the `open → closing` transition, so nothing follows a terminal delivery — a peer EOF followed by the connection's routine `.cancelled` delivers nothing further.
...
- **Capture-only trust mode (internal).** The resolved-trust enum the driver is built from has a `QUICResolvedTrust.capture` mode that `SMBQUICCertificateProbe` injects through the same driver-factory seam: its verify block stashes the presented chain (`SecTrust` → DER `[Data]`) into a lock-guarded capture slot and completes verification with `false`, so the handshake is always rejected after the capture and the probe reads the slot only after `close()` has returned. The mode is unreachable from any public configuration — `SMBQUICConfiguration`/`TrustPolicy` are unchanged and the production driver factory never resolves to it.

### Public-API note

`SMB2Manager` exposes transport selection through `transportKind` (default `.automatic` → TCP) and `quicConfiguration`, snapshotted under the manager's lock at connect start (changes never affect an in-flight or established connection). The seam types are `public`; consumers opt into QUIC via these settings. The selection surface is **Swift-only** — `SMBTransportKind`/`SMBQUICConfiguration` (and `SMBQUICCertificateProbe`) are not Objective-C-representable and are absent from the generated Objective-C interface (the existing Objective-C API is unchanged). See [docs/API.md](API.md) and, for the live interop procedure, [docs/INTEROP-QUIC.md](INTEROP-QUIC.md).
...
| `Signposts.swift` | Transport (Apple) | `InboundSignposts` — `os_signpost` emission points for the seam inbound path (`TransportRead`, `InboundChunk`, `ServiceDispatch`, `ServicePass`, `RecvDrain`) under `ro.SimpleKube.AMSMB2` / `Inbound`; guarded on `signpostsEnabled` (`#if canImport(Network)`) |
...
| `SMBQUICCertificateProbe.swift` | Public API / Transport (Apple) | `SMBQUICCertificateProbe` — capture-only server-certificate-chain probe, declared on every platform; Apple body under `#if canImport(Network)` (drives `QUICTransportApple` with a `.capture` driver factory), Linux body throws `ENOTSUP`. Also holds the internal `QUICCertificateCaptureSlot` |
