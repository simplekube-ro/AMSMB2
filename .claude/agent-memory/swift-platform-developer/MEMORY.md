...
- [patterns_transport_bridge.md](patterns_transport_bridge.md) — T5 TransportBridge: NSLock-in-async gotcha (use sync helpers), MockTransport loopback contention (separate pump startup for tests), Unmanaged lifetime discipline, `#if canImport(Network)` guard.
